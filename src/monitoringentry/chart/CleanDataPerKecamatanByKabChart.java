 ...
 * @author Toshiba
 */
public class CleanDataPerKecamatanByKabChart extends JPanel{
    
    private String kab;
    
    public CleanDataPerKecamatanByKabChart(String kab) {
        this.kab = kab;
    ...
    private CategoryDataset createDataset() {
        ...
        for(int i=0; i<listKec.size(); i++){
            ...
            double clean = B1Dao.getInstance().getCleanCountByKec(kec.getId());
            double total = B1Dao.getInstance().getByKec(kec.getId()).size();
            double cp = clean/total;
            double bcp = total-cp;
            result.addValue(cp, "Clean", kec.getNama());
            result.addValue(bcp, "Belum Clean", kec.getNama());
        }
        return result;
    }
